        ...
        <div class="card-body card-block">
            <form action="{{ route('transactions.update', $item->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="email" value="{{ old('email') ?? $item->email }}" class="form-control @error('type') is-invalid @enderror" />
                    @error('email') <div class="text-muted">{{ $message }}</div> @enderror
                </div>
                <div class="form-group">
                    <label for="number" class="form-control-label">Nomor HP</label>
                    <input type="number" name="number" value="{{ old('number') ?? $item->number }}" class="form-control @error('number') is-invalid @enderror" />
